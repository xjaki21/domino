...
void game_start(Board *board,Tessera * tessere, Tessera *speciali,int size_tessere,int size_speciali) {
    ...
    while (!selected) {
     ...
     // int input=0;
      ...
      if (input > 0 && input <= size_tessere) {
        ...
        //la funzione controlla se c'è un match e aggiunge la tessere se c'è, restituisce true o false se è stata aggiunta la tessera
 ...
 /*
  trovo la miglior tessera inseribile(quello con il punteggio più alto), se invece non è presente alcuna tessera
...
/*
creo una scacchiera per ogni tessera di quelle generate all'inizio, per ogni tessera creo una scacchiera
e trovo i best match, confronto tutte le scacchiere trovate per trova quella con più punteggio
*/
void game_start_ai(Tessera * tessere, Tessera *speciali,int size_tessere,int size_speciali){
  ...
  for(int i=0;i<size_tessere;i++){
    ...
    new_board=best_match(new_board,copia_tessere,copia_speciali,copia_size_tessere,size_speciali); 
