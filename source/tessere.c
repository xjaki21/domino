...
bool posiziona_tessera(Board *board,Tessera tessera){
  ...
  if(num_row+1>=board->size || board->rows[num_row+1].size<r->size || !board->rows[num_row+1].tessere[pos].selected){
    ...
    if(scelta=='v'){
      ...
      //estende la riga sotto per la tessera verticale se la riga superiore è più grande di dimensione della riga sotto
